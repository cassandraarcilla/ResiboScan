...
class _ScanModalState extends State<ScanModal>
  ...
  late String _folder;
  late String _image;      // always an asset path like 'assets/images/1.svg'
  Uint8List?  _photoBytes; // attached receipt photo
  ...
  void _save() {
    String? storeErr, amountErr;

    if (_store.trim().isEmpty) storeErr = 'Store name is required.';

    if (_amount.trim().isEmpty) {
      amountErr = 'Amount is required.';
    } else {
      try {
        final v = double.parse(_amount.trim());
        if (v < 0) amountErr = 'Amount cannot be negative.';
      } catch (_) {
    ...
    if (storeErr != null || amountErr != null) {
      setState(() { _storeError = storeErr; _amountError = amountErr; });
      return;
    }
    ...
    try {
      widget.onSave(Receipt(
        ...
        date      : _date,
        category  : _category,
        warranty  : _warranty.trim().isEmpty ? null : _warranty.trim(),
        image     : _image,        // always the icon asset path
        folder    : _folder,
        notes     : _notes,
        imageBytes: _photoBytes,   // the actual receipt photo
      ));
      ...
      children: [
        ...
        TextFormField(
          ...
          decoration: InputDecoration(
            hintText: hint,
            hintStyle: TextStyle(color: _inkLight.withOpacity(0.55), fontSize: 13),
            ...
            enabledBorder: OutlineInputBorder(
              ...
              borderSide: BorderSide(
                color: errorText != null ? Colors.red.shade400 : _cerulean.withOpacity(0.15),
                width: 1.5)),
            focusedBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(13),
              borderSide: BorderSide(
                color: errorText != null ? Colors.red.shade500 : _cerulean, width: 1.8)),
            filled: true,
        ...
        if (errorText != null) ...[
          ...
          Row(children: [
            ...
            Text(errorText, style: TextStyle(
              fontSize: 11.5, color: Colors.red.shade600, fontWeight: FontWeight.w500)),
          ]),
        ],
        ...
        GestureDetector(
            ...
            final picked = await showDatePicker(
              ...
              builder: (ctx, child) => Theme(
                data: Theme.of(ctx).copyWith(
                  colorScheme: ColorScheme.light(primary: _cerulean)),
                child: child!,
              ),
            );
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: _white,
              borderRadius: BorderRadius.circular(13),
              border: Border.all(color: _cerulean.withOpacity(0.15), width: 1.5)),
            child: Row(children: [
              Icon(Icons.calendar_today_rounded, size: 16, color: _cerulean.withOpacity(0.7)),
              const SizedBox(width: 10),
              Text(_date.isEmpty ? 'Select date' : _date,
                style: TextStyle(
                  fontSize: 13.5, fontWeight: FontWeight.w500,
                  color: _date.isEmpty ? _inkLight : _inkMid)),
            ]),
          ),
        ),
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _ChoiceOption(icon: Icons.camera_alt_outlined,
                      label: 'Scan with Camera', sub: 'Use your camera to capture a receipt',
                      accent: _cerulean, onTap: _pickFromCamera),
                    const SizedBox(height: 10),
                    _ChoiceOption(icon: Icons.image_outlined,
                      label: 'Upload from Gallery', sub: 'Pick an existing photo from your gallery',
                      accent: _sandy, onTap: _pickFromGallery),
                    const SizedBox(height: 10),
                    _ChoiceOption(icon: Icons.edit_outlined,
                      label: 'Enter Manually', sub: 'Type in the receipt details yourself',
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(children: [
                      GestureDetector(
                        onTap: widget.existing != null ? null : _goToChoose,
                        child: Container(
                          width: 38, height: 38,
                          decoration: BoxDecoration(
                            color: widget.existing != null
                                ? Colors.transparent
                                : _cerulean.withOpacity(0.08),
                            borderRadius: BorderRadius.circular(12),
                            border: widget.existing != null ? null : Border.all(
                    ...
                    if (_photoBytes == null)
                      GestureDetector(
                                ...
                                ListTile(
                                  leading: Container(width: 42, height: 42,
                                    decoration: BoxDecoration(
                                      ...
                                      borderRadius: BorderRadius.circular(12)),
                                    child: const Icon(Icons.image_outlined, color: _sandy, size: 20)),
                                ...
                                ListTile(
                                  leading: Container(width: 42, height: 42,
                                    decoration: BoxDecoration(
                                      ...
                                      borderRadius: BorderRadius.circular(12)),
                                    child: const Icon(Icons.camera_alt_outlined, color: _cerulean, size: 20)),
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color: _white,
                            borderRadius: BorderRadius.circular(14),
                            border: Border.all(color: _sandy.withOpacity(0.45), width: 1.5)),
                          child: Column(children: [
                            Container(width: 44, height: 44,
                              decoration: BoxDecoration(
                                color: _sandy.withOpacity(0.12),
                                shape: BoxShape.circle),
                              child: const Icon(Icons.add_photo_alternate_outlined, color: _sandy, size: 22)),
                            const SizedBox(height: 8),
                            const Text('Tap to attach image',
                              style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: _inkMid)),
                            const SizedBox(height: 3),
                            Text('Gallery or Camera',
                              style: TextStyle(fontSize: 11.5, color: _inkLight.withOpacity(0.75))),
                          ]),
                        ),
                      )
                    else
                      ClipRRect(
                        ...
                        child: Stack(children: [
                          _isSvgBytes(_photoBytes!)
                              ? SvgPicture.memory(_photoBytes!,
                        ...
                        return GestureDetector(
                          ...
                          child: AnimatedContainer(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                color: sel ? _cerulean : _cerulean.withOpacity(0.15),
                                width: sel ? 0 : 1.5),
                              boxShadow: sel ? [BoxShadow(
                                color: _cerulean.withOpacity(0.25),
                                blurRadius: 8, offset: const Offset(0, 3))] : []),
                            child: Row(mainAxisSize: MainAxisSize.min, children: [
                              Icon(icon, size: 14, color: sel ? _white : _inkMid),
                              const SizedBox(width: 5),
                    ...
                    const SizedBox(height: 10),
                    Row(
                      children: folders.where((f) => f != 'All').map((f) {
                        final sel  = _folder == f;
                        final icon = f == 'Personal' ? Icons.home_rounded : Icons.work_rounded;
                        return GestureDetector(
                          ...
                          child: AnimatedContainer(
                            ...
                            decoration: BoxDecoration(
                              ...
                              boxShadow: sel ? [BoxShadow(
                                ...
                                blurRadius: 8, offset: const Offset(0, 3))] : []),
                            child: Row(children: [
                              Icon(icon, size: 16, color: sel ? Colors.white : _inkMid),
                      ...
                      child: Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text(
                              widget.existing != null ? 'Update Receipt' : 'Save Receipt',
                              style: const TextStyle(
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: Stack(clipBehavior: Clip.none, alignment: Alignment.center, children: [
        ClipOval(
          child: SvgPicture.asset(asset, width: _sz, height: _sz, fit: BoxFit.cover,
            placeholderBuilder: (_) => Container(
              width: _sz, height: _sz,
              decoration: const BoxDecoration(
                color: Color(0x1A7A9BAA), shape: BoxShape.circle),
              child: const Icon(Icons.broken_image_outlined, size: 22, color: Color(0x807A9BAA)))),
        ),
        if (selected)
          Positioned.fill(
            child: CustomPaint(painter: _RingPainter(color: _cerulean))),
      ]),
    );
  }
}
...
class _RingPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    canvas.drawCircle(
      ...
      size.width / 2 + 4,
      Paint()..color = color..style = PaintingStyle.stroke..strokeWidth = 2.5..isAntiAlias = true,
    );
  }
  @override
...
class _SectionLabel extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
    ...
    Text(label, style: const TextStyle(
      fontSize: 12.5, fontWeight: FontWeight.w700, color: _inkMid, letterSpacing: 0.1)),
  ]);
}
...
class _ChoiceOption extends StatelessWidget {
  ...
  const _ChoiceOption({required this.icon, required this.label, required this.sub,
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: accent.withOpacity(0.07), blurRadius: 12, offset: const Offset(0, 4))]),
      child: Row(children: [
        Container(width: 46, height: 46,
          decoration: BoxDecoration(
            color: accent.withOpacity(0.10), borderRadius: BorderRadius.circular(14),
            border: Border.all(color: accent.withOpacity(0.18), width: 1)),
          child: Icon(icon, color: accent, size: 20)),
        const SizedBox(width: 14),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(label, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: _ink)),
          const SizedBox(height: 2),
          Text(sub, style: TextStyle(fontSize: 12, color: _inkLight.withOpacity(0.85))),
        ])),
        Icon(Icons.arrow_forward_ios_rounded, size: 14, color: accent.withOpacity(0.40)),
      ]),
    ),
  );
}
